...
export const fetchFeaturedProducts = async (productType: string, filterType: string): Promise<IProduct[]> => {
	"use server";
	try {
		const response = await fetch(`http://noutparts-strapi.onrender.com/api/${productType}/?populate[0]=photo&filters[tag][$in][0]=${filterType}&pagination[start]=0&pagination[limit]=2`, { cache: "no-store" });
