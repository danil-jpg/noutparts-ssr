...
export default function Page({ params }: { params: { type: categories } }) {
	...
	const breadcrumbArr: Breadcrumb[] = [
		...
		{
			label: `${params.type.toLowerCase() === "matrices" ? "Матрицы" : params.type.toLowerCase() === "batteries" ? "Аккумуляторы" : params.type.toLowerCase() === "hdds" ? "Жесткие диски" : params.type.toLowerCase() === "keyboards" ? "Клавиатуры" : params.type.toLowerCase() === "rams" ? "Оперативная память" : params.type.toLowerCase() === "power_unit" ? "Блок питания" : params.type.toLowerCase()}`,
